...
class AIAgent:
    ...
    def _save_trajectory(self, messages: List[Dict[str, Any]], user_query: str, completed: bool):
        ...
        for message in reversed(messages):
            if not isinstance(message, dict):
                continue
            raw_timestamp = message.get("timestamp") or message.get("created_at")
            if isinstance(raw_timestamp, (str, int, float)):
                turn_timestamp = raw_timestamp
                break
        workspace_id = None
        if isinstance(self.session_id, str) and ":" in self.session_id:
            workspace_id = self.session_id.split(":", 1)[0]
        metadata = {
        ...
        if self.session_log_file is not None:
            filename = str(self.session_log_file.with_name(f"{self.session_log_file.stem}.trajectory.jsonl"))
    ...
    def _save_session_log(self, messages: List[Dict[str, Any]] = None):
        ...
        try:
            ...
            metadata_only_log = bool(getattr(self, "_workspace_session_log_metadata_only", False))
            existing_payload = {}
            if self.session_log_file.exists():
                try:
                    loaded = json.loads(self.session_log_file.read_text(encoding="utf-8"))
            ...
            entry = {
                "session_id": self.session_id,
                "session_key": existing_payload.get("session_key") or self.session_id,
                "title": existing_payload.get("title"),
                "display_name": existing_payload.get("display_name") or existing_payload.get("title"),
                "source": existing_payload.get("source") or self.platform,
                "model": self.model,
                "base_url": self.base_url,
                ...
                "messages": [] if metadata_only_log else cleaned,
            }
            if metadata_only_log:
                entry["message_storage"] = "sqlite"
