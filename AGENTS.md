...
- Semantier-owned cron continuity is SQLite-authoritative. `context_from`
  must read SQLite-backed output records; if a referenced output is missing,
  raise/fail the run instead of falling back to `cron/output` files, prompt
  memory, or inferred context. Legacy files may be imported only through
  explicit migration code, not used as live fallback.
