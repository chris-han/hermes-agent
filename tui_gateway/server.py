...
def _emit_approval_request(sid: str, payload: dict | None = None):
    ...
    if "command" in data:
        try:
            ...
            data["command"] = str(data.get("command") or "")
    ...
    try:
        ...
        with _cfg_lock:
            if _cfg_cache is not None and _cfg_mtime == mtime and _cfg_path == p:
                cached = copy.deepcopy(_cfg_cache)
                try:
                    from hermes_cli import managed_scope
                    return managed_scope.apply_managed_overlay(cached)
                except Exception:
                    return cached
        if p.exists():
        ...
        with _cfg_lock:
            ...
            _cfg_mtime = mtime
            _cfg_path = p
        overlaid = copy.deepcopy(data)
        try:
            from hermes_cli import managed_scope
            return managed_scope.apply_managed_overlay(overlaid)
        except Exception:
            return overlaid
    except Exception:
        pass
            ...
            if _pending and status == "complete":
                ...
                if _pdb:
                    ...
                    try:
                        ...
                        # Invalid title — non-retryable, drop it.
