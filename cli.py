...
def _prepend_note_to_message(message: Any, note: str) -> Any:
    """Prepend a one-shot system note to text or multimodal user content."""
    cleaned = str(note or "").strip()
    if not cleaned:
        return message

    if isinstance(message, str):
        return f"{cleaned}\n\n{message}" if message else cleaned
        ...
        for part in message:
            if isinstance(part, dict):
                ...
                if not inserted and copied.get("type") == "text":
                    ...
                    copied["text"] = f"{cleaned}\n\n{text}" if text else cleaned
                    inserted = True
                parts.append(copied)
            else:
                parts.append(part)
        if not inserted:
            parts.insert(0, {"type": "text", "text": cleaned})
        return parts
...
def _resolve_prefill_messages_file(config: Dict[str, Any]) -> str:
    env_path = os.getenv("HERMES_PREFILL_MESSAGES_FILE")
    if env_path:
        return env_path
    top_level = config.get("prefill_messages_file")
    if isinstance(top_level, str) and top_level.strip():
        return top_level.strip()
    agent_cfg = config.get("agent")
    if isinstance(agent_cfg, dict):
        legacy = agent_cfg.get("prefill_messages_file")
...
_single_query_finalize_attempted_session_ids = set()
...
def _run_cleanup(*, notify_session_finalize: bool = True):
    ...
    try:
        ...
        if (
            ...
            and session_id
            and session_id not in _single_query_finalize_attempted_session_ids
        ):
            _invoke_hook(
                "on_session_finalize",
...
def _emit_interrupted_session_end(cli_obj, *, reason: str) -> None:
    """Emit session-end observer metadata for an interrupted live CLI turn."""
    ...
    try:
        ...
        pass
    session_id = getattr(agent, "session_id", None) or getattr(cli_obj, "session_id", None)
...
def _notify_single_query_session_finalize(cli_obj) -> None:
    agent = getattr(cli_obj, "agent", None)
    session_id = getattr(agent, "session_id", None) or getattr(cli_obj, "session_id", None)
    platform = getattr(agent, "platform", None) or "cli"
    if session_id:
...
def _finalize_single_query(cli_obj) -> None:
    try:
        _notify_single_query_session_finalize(cli_obj)
        _run_cleanup(notify_session_finalize=False)
    finally:
        release = getattr(cli_obj, "_release_active_session", None)
        if callable(release):
            release()
...
def _resolve_worktree_base(repo_root: str) -> tuple[str, str]:
    """Return the base ref to use for a new isolated worktree.

    Prefer the current branch's freshly fetched upstream tracking ref so
    parallel agent worktrees start from the remote tip rather than a stale
    local checkout.
    """
    import subprocess
    ...
    try:
        ...
        if upstream.returncode != 0:
            return "HEAD", "HEAD (no upstream tracking ref)"
        ...
        if "/" in base_ref:
            ...
            if fetched.returncode == 0:
                return base_ref, f"fetched {base_ref}"
            logger.debug("Failed to fetch worktree base %s: %s", base_ref, fetched.stderr.strip())
            return base_ref, f"{base_ref} (fetch failed)"

        return base_ref, base_ref
    except Exception as e:
        logger.debug("Failed to resolve worktree base: %s", e)
...
def _setup_worktree(repo_root: str = None, *, sync_base: bool = True) -> Optional[Dict[str, str]]:
    ...
    base_ref, base_label = _resolve_worktree_base(repo_root) if sync_base else ("HEAD", "HEAD")
...
class HermesCLI:
    ...
    def _paint_now(self) -> None:
        """Immediately repaint modal UI, bypassing resize/throttle guards."""
    ...
    def show_history(self):
        ...
        for msg in self.conversation_history:
            ...
            ts_suffix = ""
            if getattr(self, "show_timestamps", False) and msg.get("timestamp") is not None:
                try:
                    from datetime import datetime

                    ts_suffix = f"  [{datetime.fromtimestamp(float(msg['timestamp'])).strftime('%H:%M')}]"
                except Exception:
                    ts_suffix = ""

            if role == "user":
                print(f"\n  [You #{visible_index}]{ts_suffix}")
                print(
                    f"    {content_text[:preview_limit]}{'...' if len(content_text) > preview_limit else ''}"
                )
                continue

            print(f"\n  [Hermes #{visible_index}]{ts_suffix}")
            tool_calls = msg.get("tool_calls") or []
            if content_text:
    ...
    def _handle_branch_command(self, cmd_original: str) -> None:
        ...
        if self.agent:
            ...
            if hasattr(self.agent, "session_log_file") and hasattr(self.agent, "logs_dir"):
                workspace_hermes_home = getattr(self.agent, "_workspace_session_log_hermes_home", None)
                if isinstance(workspace_hermes_home, Path):
    ...
    def _submit_editor_buffer(self, buffer) -> None:
        ...
        if _looks_like_slash_command(message):
            ...
            try:
                ...
                buffer.reset()
            return
        target_queue = getattr(self, "_pending_input", None)
        if (
            getattr(self, "_agent_running", False)
            and getattr(self, "busy_input_mode", "interrupt") == "interrupt"
        ):
            target_queue = getattr(self, "_interrupt_queue", target_queue)
    ...
    def _show_gateway_status(self):
        ...
        try:
            ...
            print(f"    Daily reset at: {policy.at_hour}:00 UTC")
    ...
    def _show_billing(self, command: str = "/billing") -> None:
        """Render the Nous Portal billing overview."""
        from agent import billing_view as bv

        state = bv.build_billing_state()
        self._billing_overview_screen(state)

    def _show_credits(self) -> None:
        """Render Nous credit balance and top-up URL."""
        ...
        if view is None or not view.logged_in:
            print("Not logged into Nous Portal. Log in to view your credit balance and top up.")
            return
    ...
    def _billing_overview_screen(self, state) -> None:
        ...
        if state.monthly_cap is not None:
            ...
            suffix = " (default ceiling)" if cap.is_default_ceiling else ""
            print(f"  Monthly spend: {spent} of {limit} used{suffix}")
            print(f"  {self._billing_spend_bar(cap.spent_this_month_usd, cap.limit_usd)}")

        if not state.cli_billing_enabled:
            print("  Terminal billing is turned off for this org.")
        elif not state.is_admin:
            print("  Billing changes require an org admin/owner.")

        if state.portal_url:
            print(f"  Manage on portal: {state.portal_url}")
        print()

    def _billing_spend_bar(self, spent, limit) -> str:
        if not spent or not limit or limit <= 0:
            return "[░░░░░░░░░░] 0%"
        pct = max(0, min(100, int((spent / limit) * 100)))
        filled = max(0, min(10, round(pct / 10)))
        return f"[{'█' * filled}{'░' * (10 - filled)}] {pct}%"

    def _billing_limit_screen(self, state) -> None:
        from agent.billing_view import format_money

        print()
        print("  Monthly spend limit")
        cap = state.monthly_cap
        if cap is None:
            print("  No monthly spend limit data available.")
        else:
            print(
                f"  {format_money(cap.spent_this_month_usd)} of "
                f"{format_money(cap.limit_usd)} used"
            )
            print(f"  {self._billing_spend_bar(cap.spent_this_month_usd, cap.limit_usd)}")
        print("  This setting is read-only in the terminal. Manage it on the portal.")
        if state.portal_url:
            print(f"  Manage on portal: {state.portal_url}")
        print()

    def _billing_buy_flow(self, state) -> None:
        from agent.billing_view import format_money

        print()
        print("  Buy usage credits")
        if state.charge_presets:
            amounts = "  ".join(format_money(amount) for amount in state.charge_presets)
            print(f"  Presets: {amounts}")
        if state.card is not None:
            print(f"  Card: {state.card.masked}")
        print("  Purchases require the interactive CLI; use the portal from this session.")
        if state.portal_url:
            print(f"  Manage on portal: {state.portal_url}")
        print()
    ...
    def _on_tool_complete(self, tool_call_id: str, function_name: str, function_args: dict, function_result: str):
        ...
        if function_name == "delegate_task":
            try:
                payload = json.loads(function_result) if isinstance(function_result, str) else {}
                if isinstance(payload, dict) and payload.get("status") == "dispatched" and payload.get("mode") == "background":
                    ...
                    if count_int == 1:
                        _cprint(
                            f"  {_DIM}Delegate task dispatched in the background; "
                            f"the conversation will resume when it finishes.{_RST}"
                        )
                    else:
                        _cprint(
                            f"  {_DIM}{count_int} tasks dispatched in the background; "
                            f"the conversation will resume when they finish.{_RST}"
                        )
            except Exception:
                logger.debug("Delegate background notice failed for %s", tool_call_id, exc_info=True)
    ...
    def _clarify_callback(self, question, choices):
        ...
        while True:
            try:
                result = response_queue.get(timeout=1)
                self._clarify_deadline = 0
                self._print_clarify_resolution_summary(question, result)
                return result
            except queue.Empty:
    ...
    def _print_clarify_resolution_summary(self, question: str, result: str) -> None:
        if CLI_CONFIG.get("display", {}).get("persist_prompts", True) is False:
            return
        q = str(question or "")
        if len(q) > 140:
            q = q[:137] + "…"
        _cprint(f"Clarify answered: {q} -> {result}")
        ...
        with self._approval_lock:
            ...
            while True:
                try:
                    ...
                    self._approval_deadline = 0
                    self._paint_now()
                    self._print_approval_resolution_summary(result, command)
                    return result
                except queue.Empty:
    ...
    def _print_approval_resolution_summary(self, result: str, command: str) -> None:
        if CLI_CONFIG.get("display", {}).get("persist_prompts", True) is False:
            return
        labels = {
            "once": "allowed once",
        ...
        label = labels.get(result, str(result))
        preview = str(command or "")
        if len(preview) > 140:
            preview = preview[:137] + "…"
        _cprint(f"Approval {label}: {preview}")
    ...
    def _persist_active_session_before_close(self) -> None:
        """Best-effort flush of live agent messages before closing the DB session."""
        agent = getattr(self, "agent", None)
        if agent is None:
            return
        ...
        conversation = getattr(self, "conversation_history", None)
        if not isinstance(conversation, list):
            conversation = []
        transcript = getattr(agent, "_session_messages", None)
        ...
        if callable(persist):
            try:
                persist(transcript, conversation)
            except Exception:
                logger.debug("Could not persist active session before close", exc_info=True)
