...
def _normalize_skill_command_name(command: str) -> str:
    normalized = (command or "").strip().lower().replace("_", "-").replace(" ", "-")
...
def _skill_frontmatter_for_command(command: str) -> tuple[dict[str, Any], Path] | None:
...
def _classify_inactive_skill(command: str, *, platform: str | None = None) -> SlashCommandClassification | None:
...
def classify_slash_command(
    ...
    *,
    ...
    if built_in_commands and normalized_command in {
        item.lstrip("/").replace("_", "-") for item in built_in_commands
    }:
        return SlashCommandClassification(kind="built_in", command=command)

    if plugin_command_resolver is not None:
        try:
            if plugin_command_resolver(normalized_command):
...
def expand_dynamic_skill_command(
    ...
    *,
    ...
    skill_info = get_skill_commands().get(command_key) or {}
    user_instruction = remainder
    if user_instruction and user_instruction[0].isspace():
        user_instruction = user_instruction[1:]
    ...
    return DynamicSkillInvocation(
        command_key=command_key,
        skill_name=str(skill_info.get("name") or command_key.lstrip("/")),
        user_instruction=user_instruction,
