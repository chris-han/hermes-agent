...
def call_llm(
    ...
    *,
    ...
    try:
        ...
        # ── Payment / connection / rate-limit / not-found fallback ────
        # Auto mode is best-effort: if one provider fails, keep walking the
        # chain.  This includes 404 model/resource mismatches, rate limits,
        # and payment/credit exhaustion errors.
        should_fallback = (
            _is_payment_error(first_err)
            or _is_connection_error(first_err)
            or _is_rate_limit_error(first_err)
            or _is_resource_not_found_error(first_err)
        )
...
async def async_call_llm(
    ...
    *,
    ...
    try:
        ...
        # ── Payment / connection / rate-limit / not-found fallback ────
        # Auto mode is best-effort: if one provider fails, keep walking the
        # chain.  Mirrors sync call_llm fallback logic.
        should_fallback = (
            _is_payment_error(first_err)
            or _is_connection_error(first_err)
            or _is_rate_limit_error(first_err)
