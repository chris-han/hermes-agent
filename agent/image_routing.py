...
_USER_IMAGE_PLACEHOLDER_PREFIXES = ("/media/", "/mnt/data/")
_native_image_paths_lock = threading.Lock()
...
def register_native_image_paths(session_key: str, image_paths: List[str]) -> None:
    """Expose current-turn native image cache paths to tool calls.

    The gateway calls this immediately before ``agent.run_conversation()`` so
    tools invoked during the same turn can recover the real cached path even if
    the model references a synthetic placeholder like ``/media/user_image...``.
    """
    if not session_key:
        return
    normalized = [str(p) for p in image_paths if str(p).strip()]
    with _native_image_paths_lock:
        _native_image_paths_by_session[session_key] = normalized


def clear_native_image_paths(session_key: str) -> None:
    """Remove current-turn native image cache paths for *session_key*."""
    if not session_key:
        return
    with _native_image_paths_lock:
        _native_image_paths_by_session.pop(session_key, None)


def resolve_native_image_path(image_ref: str, session_key: str) -> Optional[str]:
    """Map a model-supplied image reference to the real cached image path.

    Native image turns already expose the real cache path in the text part, but
    some models still synthesize placeholder paths such as
    ``/media/user_image_20260507_083000.jpg``. For those placeholders, fall
    back to the current turn's attached image cache paths.
    """
    if not session_key or not image_ref or not isinstance(image_ref, str):
        return None

    raw_ref = image_ref.strip()
    if raw_ref.startswith("file://"):
        raw_ref = raw_ref[len("file://"):]

    expanded = str(Path(raw_ref).expanduser())
    if Path(expanded).is_file():
        return expanded

    basename = Path(raw_ref).name.lower()
    if not basename:
        return None

    with _native_image_paths_lock:
        candidates = list(_native_image_paths_by_session.get(session_key) or [])
    if not candidates:
        return None

    for candidate in candidates:
        candidate_path = Path(candidate)
        if candidate_path.name.lower() == basename and candidate_path.is_file():
            return str(candidate_path)

    looks_like_placeholder = (
        raw_ref.startswith(_USER_IMAGE_PLACEHOLDER_PREFIXES)
        or basename.startswith("user_image_")
    )
