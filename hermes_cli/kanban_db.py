...
class Task:
    ...
    # Per-task environment variable overrides injected into the worker process
    # by _default_spawn.  Stored as a JSON object (str -> str).  Allows task
    # creators running inside a workspace-bound session to forward context such
    # as SEMANTIER_WORKSPACE_RUNS_DIR to the worker without requiring the
    # dispatcher daemon to have per-workspace knowledge.
    ...
    def from_row(cls, row: sqlite3.Row) -> "Task":
        ...
        if "skills" in keys and row["skills"]:
            try:
                ...
                skills_value = None
        # Parse env_overrides JSON blob if present
        env_overrides_value: Optional[dict] = None
        if "env_overrides" in keys and row["env_overrides"]:
            try:
                parsed_env = json.loads(row["env_overrides"])
                if isinstance(parsed_env, dict):
                    env_overrides_value = {str(k): str(v) for k, v in parsed_env.items() if k}
            except Exception:
                env_overrides_value = None
        return cls(
            id=row["id"],
            title=row["title"],
            body=row["body"],
            ...
            env_overrides=env_overrides_value,
        )
...
CREATE TABLE IF NOT EXISTS tasks (
    ...
    -- larger boards.
    session_id           TEXT,
    -- Per-task environment variable overrides injected into the worker
    -- process by _default_spawn.  Stored as a JSON object (str -> str).
    -- Task creators running inside a workspace-bound session include
    -- context such as SEMANTIER_WORKSPACE_RUNS_DIR here so the kanban
    -- dispatcher daemon can forward it without per-workspace knowledge.
    env_overrides        TEXT
);
...
def _migrate_add_optional_columns(conn: sqlite3.Connection) -> None:
    ...
    if "env_overrides" not in cols:
        # Per-task environment variable overrides (JSON dict str->str).
        # Populated by task creators running inside a workspace-bound session
        # so that the kanban dispatcher can forward workspace context such as
        # SEMANTIER_WORKSPACE_RUNS_DIR to workers without the daemon itself
        # having per-workspace knowledge.  NULL on legacy rows and on tasks
        # created outside a workspace-bound session.
        _add_column_if_missing(
            conn, "tasks", "env_overrides", "env_overrides TEXT"
        )
...
def create_task(
    ...
    *,
    ...
    # Auto-capture SEMANTIER_WORKSPACE_RUNS_DIR from the creator's environment
    # so the kanban dispatcher can forward workspace context to workers even
    # when the daemon itself has no per-workspace knowledge.  Callers may pass
    # an explicit env_overrides dict; the env var is merged in only when the
    # key is absent, preserving caller intent.
    _runs_dir_key = "SEMANTIER_WORKSPACE_RUNS_DIR"
    # Read from ContextVar first (concurrent-safe), fall back to os.environ.
    try:
        import sys as _sys_kdb
        _repo_root_kdb = Path(__file__).resolve().parents[3]
        _src_kdb = str(_repo_root_kdb / "src")
        if _src_kdb not in _sys_kdb.path:
            _sys_kdb.path.insert(0, _src_kdb)
        from runtime_paths import current_workspace_runs_dir as _cwd_kdb  # type: ignore
        _creator_runs_dir = (_cwd_kdb() or "").strip()
    except Exception:
        _creator_runs_dir = os.environ.get(_runs_dir_key, "").strip()
    if _creator_runs_dir:
        if env_overrides is None:
            env_overrides = {}
        env_overrides.setdefault(_runs_dir_key, _creator_runs_dir)
    env_overrides_json = json.dumps(env_overrides) if env_overrides else None
            ...
            with write_txn(conn):
                ...
                conn.execute(
                    ...
                    INSERT INTO tasks (
                        ...
                        skills, max_retries, session_id, env_overrides
                    ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                    """,
                    (
                        task_id,
...
def _default_spawn(
    ...
    *,
    ...
    # Propagate workspace runs dir so workers write governed artifacts to the
    # correct per-workspace path rather than the shared runs root.  Three
    # resolution strategies in priority order:
    #
    # 1. env_overrides on the task (written by the task creator while running
    #    inside a workspace-bound session) — highest fidelity, always correct.
    # 2. Derive from task.tenant when it resolves to an existing Semantier
    #    workspace directory (covers tasks created by the Semantier gateway).
    # 3. Pass through the dispatcher's own env var (covers the embedded
    #    dispatcher case where the daemon runs inside a workspace-bound process).
    #
    # All env_overrides are applied first so explicit task-creator intent wins.
    if task.env_overrides:
        env.update(task.env_overrides)
    _runs_dir_key = "SEMANTIER_WORKSPACE_RUNS_DIR"
    if _runs_dir_key not in env or not env[_runs_dir_key]:
        # Strategy 2: derive from tenant if it looks like a workspace ID.
        if task.tenant:
            try:
                from pathlib import Path as _Path
                import sys as _sys
                # runtime_paths lives in the Semantier src tree, not in the
                # Hermes agent package.  Locate it relative to this file.
                _repo_root = _Path(__file__).resolve().parents[3]
                _src = str(_repo_root / "src")
                if _src not in _sys.path:
                    _sys.path.insert(0, _src)
                from runtime_paths import workspace_runs_root as _wsr
                _candidate = _wsr(task.tenant)
                if _candidate.parent.exists():
                    env[_runs_dir_key] = str(_candidate)
            except Exception:
                pass
    if _runs_dir_key not in env or not env[_runs_dir_key]:
        # Strategy 3: pass through dispatcher's context-local value (embedded dispatcher case).
        # Use ContextVar reader so concurrent dispatchers with different workspace bindings
        # don't contaminate each other via os.environ.
        try:
            from runtime_paths import current_workspace_runs_dir as _cwd_spawn  # type: ignore
            _dispatcher_val = (_cwd_spawn() or "").strip()
        except Exception:
            _dispatcher_val = os.environ.get(_runs_dir_key, "").strip()
        if _dispatcher_val:
            env[_runs_dir_key] = _dispatcher_val
    if task.branch_name:
