...
def _collect_delegate_child_ids(conn, parent_ids: List[str]) -> List[str]:
    """Collect delegate-subagent ids to cascade-delete with parent ids."""
...
def _default_db_path() -> Path:
    ...
    if shared_root:
        return Path(shared_root).expanduser().resolve() / "state.db"
...
def _is_sqlite_corruption_error(exc: Exception) -> bool:
    ...
    return isinstance(exc, sqlite3.DatabaseError) and any(
        marker in msg for marker in _SQLITE_CORRUPT_MARKERS
    )


def is_malformed_db_error(exc: Exception) -> bool:
    """Public compatibility wrapper for SQLite corruption detection."""
    return _is_sqlite_corruption_error(exc)


def _db_opens_cleanly(path: Path) -> str | None:
    """Return None when the state DB supports reads and rolled-back writes.

    The write probe intentionally inserts a temporary session/message pair so
    FTS triggers run. The transaction is rolled back before returning.
    """
    try:
        conn = sqlite3.connect(str(path), timeout=1.0, isolation_level=None)
        conn.execute("PRAGMA foreign_keys=ON")
        conn.execute("SELECT COUNT(*) FROM sessions").fetchone()
        probe_id = f"__doctor_probe_{os.getpid()}_{time.time_ns()}"
        conn.execute("BEGIN")
        try:
            conn.execute(
                """
            ...
            conn.execute(
                ...
                VALUES (?, ?, ?, ?)
                """,
                (probe_id, "user", "doctor write-health probe", time.time()),
            )
        finally:
...
def repair_state_db_schema(path: Path) -> dict[str, Any]:
    """Best-effort state DB schema repair used by doctor."""
    report: dict[str, Any] = {
        "repaired": False,
        "strategy": "sessiondb-init",
        "backup_path": None,
        "error": None,
    }
    try:
        if path.exists():
            ...
            backup = path.with_name(f"{path.name}.repair-backup-{suffix}")
            index = 1
            while backup.exists():
                backup = path.with_name(f"{path.name}.repair-backup-{suffix}.{index}")
                index += 1
...
def _quarantine_corrupt_sqlite_db(path: Path, *, db_label: str) -> Path | None:
    if not path.exists():
        return None
    suffix_index = 1
    while True:
    ...
    for suffix in ("-wal", "-shm", "-journal"):
        ...
        if sidecar.exists():
            sidecar.replace(candidate.with_name(f"{candidate.name}{suffix}"))
    logger.warning("Quarantined corrupt %s at %s to %s", db_label, path, candidate)
    return candidate
...
"""  # SCHEMA_SQL ends here — indexes are in SCHEMA_INDEX_SQL below
...
class SessionDB:
    ...
    def __init__(self, db_path: Path = None, *, _allow_corrupt_recovery: bool = True):
        ...
        try:
            ...
            try:
                ...
                pass
            if _allow_corrupt_recovery and _is_sqlite_corruption_error(exc):
                quarantined = _quarantine_corrupt_sqlite_db(self.db_path, db_label="state.db")
                if quarantined is not None:
                    retry = type(self)(
                        db_path=self.db_path,
                        _allow_corrupt_recovery=False,
                    )
    ...
    def _init_schema(self):
        ...
        # ── Indexes — created after column reconciliation ───────────────
        # Indexes that reference columns added by reconciliation must run
        # after _reconcile_columns so the columns exist when SQLite parses
        # the CREATE INDEX statement (e.g. platform_message_id).
        ...
        # Title is display metadata, not an identity key.
        # Drop any legacy uniqueness index so duplicate titles are allowed.
        try:
    ...
    def update_session_meta(self, session_id: str, model_config: str, model: str = None) -> None:
        """Update ACP session metadata without touching conversation history."""
        def _do(conn):
            conn.execute(
                """UPDATE sessions
                   SET model_config = ?,
    ...
    def set_session_title(self, session_id: str, title: str) -> bool:
        ...
        Raises ValueError if the title fails validation (too long, invalid characters).
    ...
    def get_session_by_title(self, title: str) -> Optional[Dict[str, Any]]:
        """Look up the latest session by exact title.

        Titles are not unique, so this returns the most recently started
        matching session to keep title-based flows deterministic.
        """
        with self._lock:
            cursor = self._conn.execute(
    ...
    def has_platform_message_id(self, session_id: str, platform_message_id: str) -> bool:
        """Return True if a platform message id was already persisted."""
        ...
        def _do(conn):
            cursor = conn.execute(
                """INSERT OR IGNORE INTO compression_locks
                   (session_id, holder, acquired_at)
                   VALUES (?, ?, ?)""",
                (session_id, holder, time.time()),
            )
    ...
    def get_compression_lock_holder(self, session_id: str) -> Optional[str]:
        """Return the current compression lock holder for a session, if any."""
        if not session_id:
            return None
        with self._lock:
            row = self._conn.execute(
    ...
    def release_compression_lock(self, session_id: str, holder: str) -> None:
        """Release a compression lock only when owned by *holder*."""
        if not session_id or not holder:
            return
    ...
    def archive_and_compact(self, session_id: str, messages: List[Dict[str, Any]]) -> None:
        """Archive current live rows and insert a compacted live transcript."""
    ...
    def rewind_to_message(self, session_id: str, message_id: int) -> None:
        """Mark a message and all following live rows inactive for undo/rewind."""
        ...
        def _do(conn):
            ...
            conn.execute(
                ...
                (int(row["c"] if isinstance(row, sqlite3.Row) else row[0]), session_id),
            )
    ...
    def get_messages(self, session_id: str, include_inactive: bool = False) -> List[Dict[str, Any]]:
        ...
        with self._lock:
            ...
            cursor = self._conn.execute(
                f"SELECT * FROM messages WHERE session_id = ?{active_sql} ORDER BY id",
                (session_id,),
            )
            rows = cursor.fetchall()
        result = []
    ...
    def get_messages_as_conversation(
        ...
        with self._lock:
            ...
            rows = self._conn.execute(
                ...
                f"FROM messages WHERE session_id IN ({placeholders}) AND active = 1 ORDER BY id",
                tuple(session_ids),
            ).fetchall()
    ...
    def delete_telegram_topic_binding(
        ...
        *,
        ...
        """Delete one Telegram DM topic binding and return removed row count.

        This intentionally does not create the topic-mode tables. Send-fallback
        cleanup may run for profiles that have never opted into topics.
        """
        chat_id = str(chat_id)
        ...
        def _do(conn):
            try:
                row = conn.execute(
                    """
                    SELECT user_id FROM telegram_dm_topic_bindings
                    WHERE chat_id = ? AND thread_id = ?
                    """,
                    (chat_id, thread_id),
                ).fetchone()
