...
class TestAdapterInit:
    ...
    def test_bound_request_imports_gateway_run_under_shared_runtime(
        ...
        shared_home = tmp_path / ".hermes-local"
    ...
    def test_bound_request_exports_workspace_runs_dir_and_reasserts_home(
        ...
        """Per-request binding must export `SEMANTIER_WORKSPACE_RUNS_DIR` for tool subprocesses
        and re-assert HERMES_HOME after the runtime-env reload (which calls
        ...
        # `workspace_runs_root_from_hermes_home` validates the workspace path is
        # under the repo's `workspaces/` root; redirect that root to tmp_path.
        ...
        expected_runs = workspace_home.parent / "runs"

        with _bound_request_hermes_home(str(workspace_home)):
            # Workspace binding survives the env reload.
            assert os.environ["HERMES_HOME"] == str(workspace_home.resolve())
            # Workspace runs dir is exported for tool subprocesses.
    ...
    def test_bound_request_skips_runs_env_for_shared_root(
        ...
        """When HERMES_HOME equals the shared runtime root, no per-workspace runs dir is exported."""
        import gateway.platforms.api_server as api_server
