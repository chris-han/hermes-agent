...
def test_resolve_workspace_gateway_session_creates_mapping_when_missing(tmp_path):
    ...
    first_session_id = resolve_or_create_workspace_session_id(
        hermes_home,
        workspace_id="ws-123",
        alias="web",
        source="api_server",
    ...
    with patch("runtime_paths.workspace_hermes_home_path", return_value=hermes_home):
        resolved_session_id, resolved_home = _resolve_workspace_gateway_session(
            source,
            session_id="ws-123:stale",
            session_key="agent:main:workspace:ws-123:weixin:dm:wx-user",
            source_gateway="weixin",
        )

    assert first_session_id.startswith("ws-123:session_")
    assert resolved_session_id != first_session_id
    assert resolved_session_id.startswith("ws-123:session_")
    assert resolved_home == hermes_home


def test_resolve_workspace_gateway_session_rebinds_alias_to_fresh_preferred_session(tmp_path):
    from gateway.config import Platform
    from gateway.run import _resolve_workspace_gateway_session
    from gateway.session import SessionSource
    ...
    with patch("runtime_paths.workspace_hermes_home_path", return_value=hermes_home):
        old_session_id, _ = _resolve_workspace_gateway_session(
            source,
            session_id="",
        ...
        fresh_session_id = "ws-123:session_9829c2d6900c"
        rebound_session_id, rebound_home = _resolve_workspace_gateway_session(
            source,
    ...
    assert old_session_id != fresh_session_id
    assert rebound_session_id == fresh_session_id
...
def test_resolve_workspace_gateway_session_raises_without_workspace_owner_id():
    from gateway.config import Platform
    from gateway.run import _resolve_workspace_gateway_session
    from gateway.session import SessionSource
...
def test_gateway_runner_user_sessions_enable_trajectory_export():
    ...
    assert "save_trajectories=True" in src
