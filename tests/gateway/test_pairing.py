...
def explicit_pairing_auth_db(monkeypatch, tmp_path):
...
class TestSQLiteStorage:
    def test_store_uses_sqlite_not_pairing_json_files(self, tmp_path):
        with patch("gateway.pairing.PAIRING_DIR", tmp_path):
            store = PairingStore()
        ...
        assert not (tmp_path / "telegram-pending.json").exists()
        assert not (tmp_path / "telegram-approved.json").exists()

    def test_requires_explicit_auth_db_path(self, monkeypatch, tmp_path):
...
class TestRateLimiting:
    ...
    def test_store_uses_current_hermes_home_when_module_import_path_is_stale(self, monkeypatch, tmp_path):
        current_dir = tmp_path / "current-home"
        current_dir.mkdir()
        stale_dir = tmp_path / "stale-home"
        stale_dir.mkdir()
        monkeypatch.setenv("HERMES_HOME", str(current_dir))
        monkeypatch.setattr(pairing_mod, "PAIRING_DIR", stale_dir)
        monkeypatch.setattr(pairing_mod, "_IMPORT_TIME_PAIRING_DIR", stale_dir)

        store = PairingStore()
        code = store.generate_code("telegram", "user1")

        assert isinstance(code, str) and len(code) == CODE_LENGTH
        assert Path(os.environ["SEMANTIER_AUTH_DB_PATH"]).exists()
        assert not (current_dir / "platforms" / "pairing" / "auth.db").exists()
        assert not (current_dir / "platforms" / "pairing" / "telegram-pending.json").exists()
        assert not (stale_dir / "telegram-pending.json").exists()
