...
def test_disk_guard_rejects_ungoverned_reads_and_mirrors_tmp_writes(tmp_path):
    ...
    boundary = ExecutionBoundary(
        ...
        paths=BoundaryPaths(
            ...
            uploads_root=workspace / "sessions/session-a/uploads",
    ...
    with pytest.raises(BoundaryPathRejected, match="BOUNDARY_READ_REJECTED"):
        guard.resolve_read_path(tmp_path / "other/secret.txt")

    mirrored = guard.resolve_write_path("/tmp/report.txt")
    assert mirrored == (artifacts / "tmp/report.txt").resolve()
