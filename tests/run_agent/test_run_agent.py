...
class TestRunConversation:
    ...
    def test_run_conversation_persists_trajectory_after_loop_returns(self, agent):
        self._setup_agent(agent)
        returned = {
        ...
        with (
            patch("agent.conversation_loop.run_conversation", return_value=returned) as mock_loop,
            patch.object(agent, "_save_trajectory") as mock_save,
        ):
            result = agent.run_conversation("hello")

        assert result == returned
        mock_loop.assert_called_once()
        mock_save.assert_called_once_with(returned["messages"], "hello", True)
    ...
    def test_kanban_block_called_on_iteration_exhaustion(self, agent, monkeypatch):
        ...
        with (
            ...
            patch("hermes_cli.kanban_db.connect") as mock_connect,
            patch("hermes_cli.kanban_db._record_task_failure") as mock_record_failure,
            patch.object(agent, "_persist_session"),
            patch.object(agent, "_save_trajectory"),
            patch.object(agent, "_cleanup_task_resources"),
        ):
        ...
        assert mock_record_failure.call_args.args[1] == "t_test_task_123"
        assert "Iteration budget exhausted" in mock_record_failure.call_args.kwargs["error"]
        assert mock_record_failure.call_args.kwargs["outcome"] == "timed_out"
...
class TestSaveSessionLogAtomicWrite:
    def test_uses_shared_atomic_json_helper(self, agent, tmp_path):
        ...
        with patch.dict(
            agent._save_session_log.__globals__,
            {"atomic_json_write": mock_atomic_write},
        ):
            agent._save_session_log(messages)

        mock_atomic_write.assert_called_once()
        call_args = mock_atomic_write.call_args
        assert call_args.args[0] == tmp_path / f"session_{agent.session_id}.json"
    ...
    def test_workspace_logs_are_metadata_only(self, agent, tmp_path):
        agent._session_json_enabled = True
        agent.logs_dir = tmp_path
        agent._workspace_session_log_metadata_only = True
    ...
    def test_workspace_logs_preserve_canonical_metadata(self, agent, tmp_path):
        agent._session_json_enabled = True
        agent.logs_dir = tmp_path
        agent._workspace_session_log_metadata_only = True
        agent.platform = "weixin"
        agent.session_id = "ws-123:20260603_104440_66f43674"
        (tmp_path / f"session_{agent.session_id}.json").write_text(
            json.dumps(
                {
                    "canonical_session_id": "ws-123:20260603_104440_66f43674",
                    "session_key": "agent:main:workspace:ws-123:weixin:dm:u1",
                    "platform_session_key": "agent:main:workspace:ws-123:weixin:dm:u1",
                    "chat_id": "u1",
                    "thread_id": None,
                    "origin_user_id": "u1",
                    "workspace_id": "ws-123",
                    "sandbox_key": "ws:ws-123:session:ws-123:20260603_104440_66f43674",
        ...
        payload = mock_atomic_write.call_args.args[1]
        assert payload["session_id"] == "ws-123:20260603_104440_66f43674"
        assert payload["platform"] == "weixin"
        assert payload["message_count"] == 1

    def test_workspace_logs_use_utc_timestamps(self, agent, tmp_path):
        ...
        agent._workspace_session_log_metadata_only = True
        agent.platform = "api_server"
        agent.session_start = datetime(2026, 6, 3, 10, 20, 15)
        ...
        payload = mock_atomic_write.call_args.args[1]
        assert payload["session_start"] == "2026-06-03T10:20:15"
        assert "T" in payload["last_updated"]

    def test_workspace_metadata_only_logs_do_not_rewrite_transcript_jsonl(self, agent, tmp_path):
        agent.session_log_file = tmp_path / "session_ws-123%3Asession_abc.json"
        agent._workspace_session_log_metadata_only = True
        agent.platform = "api_server"
        transcript_path = tmp_path / "ws-123%3Asession_abc.jsonl"

        with patch.dict(
            agent._save_session_log.__globals__,
            {"atomic_json_write": MagicMock()},
        ):
    ...
    def test_save_trajectory_falls_back_to_current_utc_timestamp(self, agent):
        agent.save_trajectories = True
        agent.session_id = "ws-123:demo"
        agent.platform = "weixin"
        ...
        with patch.object(
            ...
            agent._save_trajectory.__globals__,
            {"_save_trajectory_to_file": mock_save},
        ):
            agent._save_trajectory([{"role": "assistant", "content": "hi"}], "hello", True)

        mock_save.assert_called_once_with([], "test/model", True)

    def test_save_trajectory_preserves_platform_metadata(self, agent):
        agent.save_trajectories = True
        agent.session_id = "ws-123:demo"
        agent.platform = "webchat"
        agent.model = "test/model"
        agent.session_log_file = None
        ...
        with patch.object(
            ...
            agent._save_trajectory(
                [{"role": "assistant", "content": "hi", "created_at": "2026-06-03T11:23:07.964909+00:00"}],
                "hello",
                True,
            )
...
class TestMemoryNudgeCounterPersistence:
    ...
    def test_counters_not_reset_in_preamble(self):
        ...
        import inspect
        import agent.turn_context as _tc
...
class TestSessionLifecyclePersistence:
    """SessionDB status must reflect per-turn execution state."""

    def test_run_conversation_reopens_then_ends_session_db_row(self):
        import inspect
        import agent.turn_finalizer as _tf

        source = inspect.getsource(_tf)
        assert "agent._persist_session(messages, conversation_history)" in source
        assert "agent._save_trajectory(" in source
...
def test_saved_session_trajectory_row_preserves_hermes_contract(tmp_path):
    import json
    ...
    with patch.object(TrajectoryCompressor, "_init_tokenizer", lambda self: setattr(self, "tokenizer", None)):
        with patch.object(TrajectoryCompressor, "_init_summarizer", lambda self: None):
            with patch.object(
                TrajectoryCompressor,
                "compress_trajectory",
                lambda self, trajectory: (trajectory, TrajectoryMetrics()),
            ):
                processed, metrics = TrajectoryCompressor(CompressionConfig()).process_entry(entry)
    assert processed["conversations"] == trajectory
