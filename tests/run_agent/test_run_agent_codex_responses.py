...
def test_build_api_kwargs_codex(monkeypatch):
    ...
    terminal_tool = next(tool for tool in kwargs["tools"] if tool["name"] == "terminal")
    assert terminal_tool["type"] == "function"
    assert terminal_tool["strict"] is False
    assert "function" not in terminal_tool
    assert kwargs["store"] is False
...
def test_run_codex_stream_retries_when_completed_event_missing(monkeypatch):
    ...
    response = agent._run_codex_stream(_codex_request_kwargs(), client=agent.client)
    assert calls["stream"] == 0
    assert response.output[0].content[0].text == "fallback"
...
def test_try_refresh_codex_client_credentials_handles_xai_oauth(monkeypatch):
    ...
    def _fake_create_openai_client(client_kwargs, *, reason, shared=True):
        rebuilt["kwargs"] = dict(client_kwargs)
    ...
    monkeypatch.setattr(agent, "_create_openai_client", _fake_create_openai_client)
    ...
    assert rebuilt["reason"] == "xai-oauth_credential_refresh"
...
def test_try_refresh_codex_client_credentials_rebuilds_client(monkeypatch):
    ...
    assert rebuilt["reason"] == "openai-codex_credential_refresh"
    assert rebuilt["shared"] is True
    assert isinstance(agent.client, _RebuiltClient)
