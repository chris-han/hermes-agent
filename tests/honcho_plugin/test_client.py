...
class TestResolveSessionNameGatewayKey:
    ...
    def test_title_remaps_gateway_key(self):
        """A title can intentionally remap the active Honcho session."""
        config = HonchoClientConfig(session_strategy="per-session")
        result = config.resolve_session_name(
...
class TestResetHonchoClient:
    def test_reset_clears_singleton(self):
        ...
        # Seed the cached client, then verify reset_honcho_client() clears it.
