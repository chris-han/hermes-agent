...
def test_get_nous_auth_status_reports_revoked_refresh_session(tmp_path, monkeypatch):
    ...
    def _boom(min_key_ttl_seconds=60):
        raise auth_mod.AuthError("Refresh session has been revoked", provider="nous", relogin_required=True)

    monkeypatch.setattr(auth_mod, "resolve_nous_runtime_credentials", _boom)

    status = auth_mod.get_nous_auth_status()
...
def test_refresh_token_persisted_when_mint_returns_insufficient_credits(tmp_path, monkeypatch):
    ...
    def _fake_mint_agent_key(*, client, portal_base_url, access_token, min_ttl_seconds):
        ...
        if mint_calls["count"] == 1:
            raise auth_mod.AuthError("credits exhausted", provider="nous", code="insufficient_credits")
...
def test_terminal_refresh_failure_quarantines_tokens(
    ...
    with pytest.raises(auth_mod.AuthError, match="Refresh session has been revoked"):
        auth_mod.resolve_nous_runtime_credentials(min_key_ttl_seconds=300)
    ...
    with pytest.raises(auth_mod.AuthError, match="No access token found"):
        auth_mod.resolve_nous_runtime_credentials(min_key_ttl_seconds=300)
...
def test_managed_access_token_refresh_failure_quarantines_tokens(
    ...
    with pytest.raises(auth_mod.AuthError, match="Invalid refresh token"):
...
def test_mint_retry_uses_latest_rotated_refresh_token(tmp_path, monkeypatch):
    ...
    def _fake_mint_agent_key(*, client, portal_base_url, access_token, min_ttl_seconds):
        mint_calls["count"] += 1
        if mint_calls["count"] == 1:
            raise auth_mod.AuthError("stale access token", provider="nous", code="invalid_token")
...
def test_refresh_token_exchange_sends_refresh_token_header():
    ...
    payload = auth_mod._refresh_access_token(
        client=client,
        portal_base_url="https://portal.nousresearch.com",
        client_id="hermes-cli",
        refresh_token="refresh-1",
