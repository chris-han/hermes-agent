...
def profile_env(tmp_path, monkeypatch):
    ...
    * Path.home() -> tmp_path  (so _get_profiles_root() = tmp_path/.semantier-home/profiles)
    * HERMES_HOME  -> tmp_path/.semantier-home  (so get_hermes_home() agrees)
    * Creates the bare-minimum ~/.semantier-home directory.
    """
    monkeypatch.setattr(Path, "home", lambda: tmp_path)
    default_home = tmp_path / ".semantier-home"
...
class TestCreateProfile:
    ...
    def test_clone_all_excludes_sibling_profiles_tree(self, profile_env):
        """--clone-all from default ~/.semantier-home must not copy profiles/* (nested explosion)."""
        tmp_path = profile_env
        default_home = tmp_path / ".semantier-home"
...
class TestActiveProfile:
    ...
    def test_empty_file_returns_default(self, profile_env):
        tmp_path = profile_env
        active_path = tmp_path / ".semantier-home" / "active_profile"
...
class TestGetActiveProfileName:
    ...
    def test_default_hermes_home_returns_default(self, profile_env):
        # HERMES_HOME points to tmp_path/.semantier-home which is the default
        assert get_active_profile_name() == "default"

    def test_profile_path_returns_profile_name(self, profile_env, monkeypatch):
        tmp_path = profile_env
        create_profile("coder", no_alias=True)
        profile_dir = tmp_path / ".semantier-home" / "profiles" / "coder"
...
class TestResolveProfileEnv:
    ...
    def test_existing_profile_returns_path(self, profile_env):
        ...
        assert result == str(tmp_path / ".semantier-home" / "profiles" / "coder")

    def test_default_returns_default_home(self, profile_env):
        tmp_path = profile_env
        result = resolve_profile_env("default")
        assert result == str(tmp_path / ".semantier-home")
...
class TestRenameProfile:
    ...
    def test_renames_directory(self, profile_env):
        ...
        old_dir = tmp_path / ".semantier-home" / "profiles" / "oldname"
...
class TestInternalHelpers:
    ...
    def test_profiles_root_under_home(self, profile_env):
        ...
        assert root == tmp_path / ".semantier-home" / "profiles"

    def test_default_hermes_home(self, profile_env):
        tmp_path = profile_env
        home = _get_default_hermes_home()
        assert home == tmp_path / ".semantier-home"

    def test_profiles_root_docker_deployment(self, tmp_path, monkeypatch):
        """In Docker (HERMES_HOME outside ~/.semantier-home), profiles go under HERMES_HOME."""
    ...
    def test_profiles_root_profile_mode(self, tmp_path, monkeypatch):
        """In profile mode (HERMES_HOME under ~/.semantier-home), profiles root is still ~/.semantier-home/profiles."""
        native = tmp_path / ".semantier-home"
...
class TestEdgeCases:
    ...
    def test_create_profile_returns_correct_path(self, profile_env):
        ...
        expected = tmp_path / ".semantier-home" / "profiles" / "mybot"
        assert result == expected
