...
def _patch_nous_runtime_resolver(*args, **kwargs):
    return patch.dict(
        NousPortalAdapter._get_credential.__globals__,
        {
            "resolve_nous_runtime_credentials": MagicMock(*args, **kwargs),
        },
    )
...
def test_nous_adapter_get_credential_uses_runtime_resolver(tmp_path, monkeypatch):
    ...
    with _patch_nous_runtime_resolver(return_value=refreshed_state):
        mock_resolve = NousPortalAdapter._get_credential.__globals__[
...
def test_nous_adapter_retry_credential_skips_opaque_bearer(tmp_path, monkeypatch):
    ...
    with _patch_nous_runtime_resolver():
        mock_resolve = NousPortalAdapter._get_credential.__globals__[
            "resolve_nous_runtime_credentials"
        ]
        adapter = NousPortalAdapter()
...
def test_nous_adapter_concurrent_refresh_serialized(tmp_path, monkeypatch):
    ...
    with _patch_nous_runtime_resolver(side_effect=serializing_refresh):
