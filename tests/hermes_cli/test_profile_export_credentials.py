...
class TestCredentialExclusion:
    ...
    def test_named_profile_export_excludes_auth(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setitem(export_profile.__globals__, "_get_profiles_root", lambda: profiles_root)
        monkeypatch.setitem(export_profile.__globals__, "get_profile_dir", lambda n: profile_dir)
        monkeypatch.setitem(export_profile.__globals__, "validate_profile_name", lambda n: None)
