...
def _arrange_startup_fallback(monkeypatch, tmp_path, running_pids, *, start_now=False):
    ...
    monkeypatch.setattr(
        ...
        lambda *args, **kwargs: (start_now, True),
    )
...
def test_install_prompts_start_choices_before_uac(monkeypatch, tmp_path, capsys):
    ...
    monkeypatch.setattr(
        "hermes_cli.setup.prompt_yes_no",
        lambda prompt, default=True: calls.append(("prompt", prompt, default)) or next(answers),
    )
    monkeypatch.setattr(
        gateway_windows,
        "_launch_elevated_install",
...
def test_install_startup_fallback_does_not_spawn_when_gateway_already_running(monkeypatch, tmp_path, capsys):
    ...
    script_path, calls = _arrange_startup_fallback(
        monkeypatch,
        tmp_path,
        [24476],
        start_now=True,
