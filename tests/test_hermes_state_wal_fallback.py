...
class TestSessionDbUsesWalFallback:
    ...
    def test_sessiondb_quarantines_malformed_state_db_and_recreates(self, tmp_path):
        """A corrupt state.db should be preserved and replaced with a usable DB."""
        target = tmp_path / "state.db"
        target.write_bytes(b"SQLite format 3\x00" + b"not a valid sqlite database")
        ...
        quarantined = tmp_path / "state.db.corrupt.1"
