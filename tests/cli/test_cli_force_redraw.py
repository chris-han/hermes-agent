...
class TestForceFullRedraw:
    ...
    def test_resize_recovery_resets_renderer_without_clearing_screen(self, bare_cli):
        """Resize recovery resets prompt_toolkit state without clearing scrollback."""
        app = MagicMock()
        events = []
        app.renderer.reset.side_effect = lambda **_: events.append("renderer_reset")
        app.invalidate.side_effect = lambda: events.append("invalidate")
        ...
        assert events == ["renderer_reset", "invalidate", "original_resize"]
        app.renderer.reset.assert_called_once_with(leave_alternate_screen=False)
        app.invalidate.assert_called_once()
    ...
    def test_resize_recovery_never_wipes_scrollback(self, bare_cli):
        """Resize recovery must be banner-safe and leave scrollback intact."""
        app = MagicMock()
        events = []
