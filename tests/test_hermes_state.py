...
class TestMessageStorage:
    ...
    def test_append_message_preserves_supplied_timestamp(self, db):
        db.create_session(session_id="s1", source="cli")

        db.append_message("s1", role="user", content="Hello", timestamp=1781437200.25)

        messages = db.get_messages("s1")
        assert messages[0]["timestamp"] == 1781437200.25
