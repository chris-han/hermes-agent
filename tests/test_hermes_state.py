...
class TestSchemaInit:
    ...
    def test_migration_drops_legacy_unique_title_index(self, tmp_path):
        """Opening a legacy DB removes the unique title index and allows duplicates."""
        import sqlite3

        db_path = tmp_path / "legacy_unique_title.db"
        conn = sqlite3.connect(str(db_path))
        conn.executescript("""
            CREATE TABLE schema_version (version INTEGER NOT NULL);
        ...
        # Legacy unique index is removed on startup.
        index_row = migrated_db._conn.execute(
            "SELECT name FROM sqlite_master WHERE type='index' AND name='idx_sessions_title_unique'"
        ).fetchone()
        assert index_row is None

        migrated_db.create_session("s1", "cli")
        migrated_db.create_session("s2", "cli")
        assert migrated_db.set_session_title("s1", "same") is True
        assert migrated_db.set_session_title("s2", "same") is True
...
class TestSessionTitles:
    """Tests for title behavior and title-based lookups."""

    def test_duplicate_title_allowed(self, db):
        """Different sessions may share the same title."""
        db.create_session("s1", "cli")
        db.create_session("s2", "cli")
        assert db.set_session_title("s1", "my project") is True
        assert db.set_session_title("s2", "my project") is True
        assert db.get_session_title("s1") == "my project"
        assert db.get_session_title("s2") == "my project"
    ...
    def test_get_session_by_title_returns_latest_match(self, db):
        db.create_session("s1", "cli")
        db.set_session_title("s1", "shared")
        db.create_session("s2", "cli")
        db.set_session_title("s2", "shared")
        result = db.get_session_by_title("shared")
        assert result is not None
        assert result["id"] == "s2"
