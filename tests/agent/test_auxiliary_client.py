...
class TestAuxiliaryTaskExtraBody:
    def test_title_generation_extra_body_disables_thinking(self):
        """DEFAULT_CONFIG must ship enable_thinking=False for title_generation.

        qwen3 family models (e.g. qwen3.5-plus) default to thinking/reasoning
        mode which adds 30+ seconds of latency to a short auxiliary request.
        The default config must disable it so the fix is product-level and
        applies to every fresh install, not just to hand-edited local configs.

        This test imports DEFAULT_CONFIG directly (no mocking) so it fails
        if someone removes or weakens the default.
        """
        from hermes_cli.config import DEFAULT_CONFIG

        title_cfg = DEFAULT_CONFIG.get("auxiliary", {}).get("title_generation", {})
        extra_body = title_cfg.get("extra_body", {})
        assert isinstance(extra_body, dict), (
            "auxiliary.title_generation.extra_body must be a dict in DEFAULT_CONFIG"
        )
        assert extra_body.get("enable_thinking") is False, (
            "auxiliary.title_generation.extra_body.enable_thinking must be False in DEFAULT_CONFIG — "
            "qwen3 family models default to thinking mode which causes >30s latency on title generation"
        )

    def test_title_generation_extra_body_is_forwarded_to_provider(self):
        """extra_body from task config must be forwarded to the provider call.

        This proves the plumbing: _get_task_extra_body reads the config and
        call_llm forwards it as extra_body to chat.completions.create().
        """
        client = MagicMock()
        ...
        with patch("hermes_cli.config.load_config", return_value=config), patch(
            "agent.auxiliary_client._get_cached_client",
            return_value=(client, "qwen3.5-plus"),
        ):
            call_llm(
                task="title_generation",
                messages=[
        ...
        kwargs = client.chat.completions.create.call_args.kwargs
        assert "extra_body" in kwargs, "extra_body must be forwarded to the provider"
        assert kwargs["extra_body"].get("enable_thinking") is False
...
class TestAuxUnhealthyCache:
    ...
    def test_ttl_expiry_evicts(self, monkeypatch):
        ...
        now = 1_000.0
        monkeypatch.setattr(aux_mod.time, "time", lambda: now)
