...
class TestDefaultContextLengths:
    def test_claude_models_context_lengths(self):
        for key, value in DEFAULT_CONTEXT_LENGTHS.items():
            ...
            if any(tag in key for tag in ("fable", "4.6", "4-6", "4.7", "4-7", "4.8", "4-8")):
...
class TestGetModelContextLength:
    def test_known_model_from_api(self, monkeypatch):
        import agent.model_metadata as mm

        monkeypatch.setattr(
            mm,
            "fetch_model_metadata",
            lambda: {"test/model": {"context_length": 32000}},
        )
        assert mm.get_model_context_length("test/model") == 32000

    @patch("agent.model_metadata.fetch_model_metadata")
    def test_fallback_to_defaults(self, mock_fetch):
        mock_fetch.return_value = {}
        assert get_model_context_length("anthropic/claude-sonnet-4") in {200000, 1000000}
    ...
    def test_qwen3_coder_context_length(self, mock_fetch):
        """qwen3-coder must not fall back to the generic 128K Qwen default."""
        mock_fetch.return_value = {}
        assert get_model_context_length("qwen3-coder") in {262144, 1048576}
    ...
    def test_qwen3_6_plus_context_length(self, mock_fetch):
        ...
        mock_fetch.return_value = {}
        assert get_model_context_length("qwen3.6-plus") in {1000000, 1048576}
        ...
        assert get_model_context_length("qwen/qwen3.6-plus") in {1000000, 1048576}
        assert get_model_context_length("dashscope/qwen3.6-plus") in {1000000, 1048576}
    ...
    def test_custom_endpoint_metadata_beats_fuzzy_default(self, monkeypatch):
        import agent.model_metadata as mm
    ...
    def test_config_context_length_zero_is_ignored(self, mock_fetch):
        ...
        assert result in {200000, 1000000}
...
class TestBedrockContextResolution:
    ...
    def test_non_bedrock_url_still_probes(self, monkeypatch):
        ...
        def _fake_fetch(*args, **kwargs):
            ...
            return {"some-model": {"context_length": 50000}}

        monkeypatch.setattr(mm, "fetch_endpoint_model_metadata", _fake_fetch)
        ctx = mm.get_model_context_length(
            "some-model",
...
class TestStripProviderPrefix:
    ...
    def test_ollama_model_tag_not_mangled_in_context_lookup(self, monkeypatch):
        ...
        monkeypatch.setattr(
            mm,
            "fetch_endpoint_model_metadata",
            lambda *args, **kwargs: {"qwen3.5:27b": {"context_length": 32768}},
        )
        monkeypatch.setattr(mm, "_is_custom_endpoint", lambda *args, **kwargs: True)
        result = mm.get_model_context_length(
            "qwen3.5:27b",
...
class TestFetchModelMetadata:
    ...
    @patch("agent.model_metadata.requests.get")
    def test_api_failure_returns_empty_on_cold_cache(self, mock_get, tmp_path, monkeypatch):
        import agent.model_metadata as mm
        ...
        monkeypatch.setattr(
            ...
            lambda: tmp_path / "openrouter_model_metadata.json",
        )
    ...
    def test_ttl_expiry_triggers_refetch(self, mock_get, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(mm, "_model_metadata_disk_cache_age_seconds", lambda: None)
...
class TestContextLengthCache:
    ...
    def test_idempotent_save(self, tmp_path):
        ...
        cache_file = tmp_path / "cache.yaml"
        with patch("agent.model_metadata._get_context_cache_path", return_value=cache_file):
            mm.save_context_length("model", "http://x", 32768)
            mm.save_context_length("model", "http://x", 32768)
