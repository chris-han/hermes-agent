...
class TestDecideImageInputMode:
    ...
    def test_auto_with_vision_capable_model(self):
        cfg = {
            "providers": {
                "anthropic": {
                    "models": {
                        "claude-sonnet-4": {"supports_vision": True},
                    },
                },
            },
        }
    ...
    def test_none_config_is_auto(self):
        assert decide_image_input_mode("openrouter", "brand-new-slug", None) == "text"

    def test_invalid_mode_coerces_to_auto(self):
        cfg = {
            "agent": {"image_input_mode": "weird-value"},
            "providers": {
                "anthropic": {
                    "models": {
                        "claude-sonnet-4": {"supports_vision": True},
...
# ─── Native image path registry ──────────────────────────────────────────────


class TestNativeImageRegistry:
    def test_placeholder_resolves_to_current_session_image(self, tmp_path: Path):
        img = tmp_path / "invoice.jpg"
        img.write_bytes(b"\xff\xd8\xff\xe0\x00\x10JFIF\x00\x01" + b"\x00" * 32)
        register_native_image_paths("test-session", [str(img)])

        resolved = resolve_native_image_path(
    ...
    def test_exact_cached_basename_match_wins(self, tmp_path: Path):
        img = tmp_path / "img_abcdef123456.jpg"
        img.write_bytes(b"\xff\xd8\xff\xe0\x00\x10JFIF\x00\x01" + b"\x00" * 32)
        register_native_image_paths("test-session", [str(img)])
    ...
    def test_clear_native_image_paths_removes_session_entry(self, tmp_path: Path):
        img = tmp_path / "invoice.jpg"
        img.write_bytes(b"\xff\xd8\xff\xe0\x00\x10JFIF\x00\x01" + b"\x00" * 32)
        register_native_image_paths("test-session", [str(img)])
        clear_native_image_paths("test-session")
