...
def test_api_server_delivery_uses_semantier_runtime_session_db(monkeypatch, tmp_path):
    from cron.scheduler import _deliver_api_server_session_result
    ...
    runtime_db = SessionDB(db_path=runtime_home / "state.db")
    workspace_db = SessionDB(db_path=workspace_home / "state.db")
    try:
        runtime_db.create_session(session_id, "api_server")

        error = _deliver_api_server_session_result(
            {
                "id": "job-1",
                "origin": {
                    "platform": "api_server",
                    "session_id": session_id,
                    "hermes_home": str(workspace_home),
                },
            },
            "Cronjob Response: hello",
        )

        assert error is None
        assert [
            message["content"] for message in runtime_db.get_messages(session_id)
        ] == ["Cronjob Response: hello"]
        assert workspace_db.get_session(session_id) is None
    finally:
        runtime_db.close()
        workspace_db.close()


def test_platform_origin_delivery_appends_to_runtime_session_db(monkeypatch, tmp_path):
    runtime_home = tmp_path / ".semantier-home"
    runtime_home.mkdir()
    monkeypatch.setenv("SEMANTIER_LOCAL_STATE_DIR", str(runtime_home))

    session_id = "ws-123:20260615_014130_726d9d51"
    db = SessionDB(db_path=runtime_home / "state.db")
    try:
        db.create_session(session_id, "weixin")
    finally:
        db.close()
    ...
    with (
        ...
        error = _deliver_result(
            {
                "id": "weixin-origin-job",
                "name": "Weixin Origin Job",
                "deliver": "origin",
                "origin": {
                    "platform": "weixin",
                    "chat_id": "wx-chat-1",
                    "session_id": session_id,
                    "session_key": "agent:main:workspace:ws-123:weixin:dm:wx-chat-1",
                },
            },
            "scheduled weixin output",
        )
    ...
    assert messages[0]["content"] == "scheduled weixin output"


def test_successful_delivered_job_does_not_need_markdown_output():
    assert (
        _should_save_job_output(
            {"id": "job-1", "deliver": "origin"},
            success=True,
            should_deliver=True,
            delivery_error=None,
        )
...
def test_successful_local_job_keeps_markdown_output():
    assert (
        _should_save_job_output(
            {"id": "job-1", "deliver": "local"},
            success=True,
...
def test_tick_known_homes_ticks_each_workspace_home(monkeypatch, tmp_path):
    homes = [
        tmp_path / "ws-a" / ".semantier-home",
        tmp_path / "ws-b" / ".semantier-home",
    ]
    seen = []

    monkeypatch.setattr("cron.scheduler._known_cron_homes", lambda: homes)

    def fake_tick(*, verbose=True, adapters=None, loop=None):
    ...
    assert tick_known_homes(verbose=False) == 2
    assert seen == [home.resolve() for home in homes]
...
class TestResolveOrigin:
    ...
    def test_api_server_session_origin(self):
        job = {
            "origin": {
                "platform": "api_server",
...
class TestResolveDeliveryTarget:
    ...
    def test_origin_delivery_targets_api_server_session(self):
        assert _resolve_delivery_target({
            "deliver": "origin",
            "origin": {
                "platform": "api_server",
                "session_id": "ws-123:session_api",
            ...
            "platform": "api_server",
            "chat_id": "ws-123:session_api",
            "session_id": "ws-123:session_api",
        }

    def test_origin_delivery_preserves_weixin_adapter_keys(self):
        assert _resolve_delivery_target({
            "deliver": "origin",
            "origin": {
                "platform": "weixin",
    ...
    def test_origin_delivery_resolves_weixin_adapter_key_from_session_metadata(self):
        with patch(
            "agents.workspace_session_logs.resolve_workspace_session_delivery_adapter_key",
            return_value="weixin:ws-123:acct-1@im.bot",
        ) as resolve_mock:
            assert _resolve_delivery_target({
                "id": "wx-job",
                "deliver": "origin",
                "origin": {
                    "platform": "weixin",
                    "chat_id": "wx-user-1",
                    "session_id": "ws-123:20260615_020304_a28acb33",
                    "session_key": "agent:main:workspace:ws-123:weixin:dm:wx-user-1",
                    "hermes_home": "/tmp/ws-123/.hermes",
                },
            }) == {
                "platform": "weixin",
                "chat_id": "wx-user-1",
    ...
    def test_origin_delivery_without_origin_does_not_fall_back_to_home_channel(
        self, monkeypatch
    ):
        for fallback_env in (
            "MATRIX_HOME_ROOM",
    ...
    def test_origin_delivery_without_raw_origin_uses_user_scoped_weixin_binding(
        self, monkeypatch, tmp_path
    ):
        monkeypatch.setenv(
            "SEMANTIER_LOCAL_STATE_DIR", str(tmp_path / ".semantier-home")
        )

        from agents.auth_db import ensure_auth_db, save_weixin_runtime_account
...
class TestApiServerSessionDelivery:
    def test_deliver_result_appends_to_api_server_session_db(self, tmp_path):
        ...
        assert len(messages) == 1
        assert messages[0]["role"] == "assistant"
        assert "scheduled output" in messages[0]["content"]
...
class TestDeliverResultWrapping:
    ...
    def test_live_weixin_delivery_resolves_account_scoped_adapter(self):
        from gateway.config import Platform
        from concurrent.futures import Future

        account_adapter = AsyncMock()
        account_adapter.send.return_value = MagicMock(success=True)

        proxy_adapter = MagicMock()

        def resolve_for_source(source):
        ...
        proxy_adapter.resolve_for_source.side_effect = resolve_for_source
        ...
        with (
            ...
            patch(
                "tools.send_message_tool._send_to_platform", new=AsyncMock()
            ) as send_mock,
        ):
            error = _deliver_result(
                job,
                "hello weixin",
                adapters={Platform.WEIXIN: proxy_adapter},
                loop=loop,
            )

        assert error is None
        proxy_adapter.resolve_for_source.assert_called_once()
        account_adapter.send.assert_called_once()
        assert account_adapter.send.call_args[0][:2] == ("wx-user-1", "hello weixin")
        assert account_adapter.send.call_args.kwargs["metadata"] == {
            "adapter_key": "weixin:ws-123:acct-1@im.bot",
            "delivery_adapter_key": "weixin:ws-123:acct-1@im.bot",
        }
...
class TestRunJobConfigEnvVarExpansion:
    ...
    def _runtime_for_request(self, **kwargs):
        runtime = dict(self._RUNTIME)
        runtime["model"] = kwargs.get("target_model") or runtime["model"]
        return runtime
    ...
    def test_model_env_ref_in_config_yaml_is_expanded(self, tmp_path, monkeypatch):
        ...
        with (
            ...
            patch(
                "hermes_cli.runtime_provider.resolve_runtime_provider",
                side_effect=self._runtime_for_request,
            ),
            patch("run_agent.AIAgent") as mock_agent_cls,
        ):
            mock_agent = MagicMock()
...
class TestSilentDelivery:
    ...
    def test_sqlite_output_saved_even_when_markdown_suppressed(self):
        with (
            patch("cron.scheduler.get_due_jobs", return_value=[self._make_job()]),
            patch(
                "cron.scheduler.run_job",
            ...
            patch("cron.scheduler.save_job_output") as save_mock,
            patch("cron.scheduler.save_job_output_record") as save_record_mock,
        ...
        save_record_mock.assert_called_once_with("monitor-job", "# full output")
