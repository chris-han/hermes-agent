...
class TestJobCRUD:
    def test_create_uses_active_hermes_home_when_globals_unpatched(self, tmp_path, monkeypatch):
        ...
        workspace_home = tmp_path / "workspaces" / "ws-1" / ".hermes"
        monkeypatch.setattr(jobs_mod, "HERMES_DIR", jobs_mod._DEFAULT_HERMES_DIR)
        monkeypatch.setattr(jobs_mod, "CRON_DIR", jobs_mod._DEFAULT_CRON_DIR)
        monkeypatch.setattr(jobs_mod, "JOBS_FILE", jobs_mod._DEFAULT_JOBS_FILE)
        monkeypatch.setattr(jobs_mod, "OUTPUT_DIR", jobs_mod._DEFAULT_OUTPUT_DIR)

        token = set_active_hermes_home(workspace_home)
        try:
            job = create_job(prompt="tenant scoped", schedule="30m")
        finally:
            reset_active_hermes_home(token)

        jobs_path = workspace_home / "cron" / "jobs.json"
...
class TestUpdateJob:
    ...
    def test_update_oneshot_to_recurring_clears_auto_repeat_once(self, tmp_cron_dir):
        job = create_job(prompt="Report time", schedule="1m")
        assert job["schedule"]["kind"] == "once"
        assert job["repeat"]["times"] == 1
        ...
        fetched = get_job(job["id"])
        assert fetched is not None
        assert fetched["repeat"]["completed"] == 1
        assert fetched["next_run_at"] is not None

    def test_update_repeat_zero_sets_forever_policy(self, tmp_cron_dir):
        job = create_job(prompt="Monitor", schedule="every 1h", repeat=3)
        assert job["repeat"]["times"] == 3

        updated = update_job(job["id"], {"repeat": 0})

        assert updated is not None
        assert updated["repeat"] == {"times": None, "completed": 0}

    def test_update_repeat_positive_resets_completed_counter(self, tmp_cron_dir):
        job = create_job(prompt="Monitor", schedule="every 1h", repeat=0)
        mark_job_run(job["id"], success=True)
        assert get_job(job["id"])["repeat"]["completed"] == 1

        updated = update_job(job["id"], {"repeat": 5})

        assert updated is not None
        assert updated["repeat"] == {"times": 5, "completed": 0}
