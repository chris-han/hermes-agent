...
def terminal_tool(
    ...
    try:
        ...
        # Cross-tool governance: redirect any shell payload that targets
        # `.hermes-local/runs/<sub>` to the bound workspace runs folder
        # (`workspaces/<id>/runs/<sub>`). Refuse only when no workspace
        # authority is resolvable. See file_tools.redirect_shared_runs_in_text.
        try:
            from tools.file_tools import redirect_shared_runs_in_text as _redirect_text
        except ImportError:
            _redirect_text = None
        if _redirect_text is not None:
            rewritten, info = _redirect_text(command)
