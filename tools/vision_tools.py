...
def _resolve_attached_image_path(image_ref: str) -> str:
    """Map model-supplied placeholders to the current turn's cached image.

    Native multimodal turns attach user images inline as ``data:...`` parts and
    separately expose the real cache path in the text hint. Some models still
    synthesize placeholders like ``/media/user_image_20260507_083000.jpg`` when
    they later call ``vision_analyze``. Resolve those placeholders back to the
    current turn's cached image path before local-file validation.
    """
    ...
    try:
        from agent.image_routing import resolve_native_image_path
        from tools.approval import get_current_session_key

        session_key = get_current_session_key(default="")
        resolved = resolve_native_image_path(raw_ref, session_key=session_key)
        if resolved:
            return resolved
    except Exception as exc:  # pragma: no cover - defensive fallback
        logger.debug("Unable to resolve attached image placeholder %r: %s", image_ref, exc)
    return image_ref
...
async def vision_analyze_tool(
    ...
    try:
        ...
        resolved_url = _resolve_attached_image_path(image_url)
        if resolved_url.startswith("file://"):
            resolved_url = resolved_url[len("file://"):]
        debug_call_data["parameters"]["resolved_image_url"] = resolved_url
        local_path = Path(os.path.expanduser(resolved_url))
        if local_path.is_file():
            # Local file path (e.g. from platform image cache) -- skip download
            logger.info("Using local image file: %s", resolved_url)
            temp_image_path = local_path
