"""Bundled Hermes memory provider backed by native Semantica workspace state."""
...
SEMANTICA_STATE_SCHEMA = {
    ...
    "description": "Inspect the active workspace-scoped Semantica memory state.",
    "parameters": {
        "type": "object",
        "properties": {
            "include_paths": {
...
SEMANTICA_GRAPH_SCHEMA = {
    ...
    "description": "Mutate or query the active workspace-scoped Semantica ContextGraph.",
    "parameters": {
        "type": "object",
        "required": ["action"],
        "properties": {
...
class SemanticaMemoryProvider(MemoryProvider):
    """Hermes provider that owns real Semantica state inside a workspace root."""
    ...
    def backup_paths(self) -> List[str]:
        return [str(self._workspace_root)] if self._workspace_root else []

    def _handle_graph(self, args: Dict[str, Any]) -> str:
        if not self._graph or not self._control_adapter:
            return tool_error("Semantica provider is not initialized")
    ...
    def _workspace_state(self, include_paths: bool) -> Dict[str, Any]:
        ...
        if include_paths:
            state["workspace_root"] = str(self._workspace_root)
            state["state_root"] = str(self._state_root)
            state["graph_path"] = str(self._graph_path)
            state["graph_versions_path"] = str(
                self._control_adapter.semantica_state_root / "graph_versions.sqlite3"
            )
            state["ontology_versions_path"] = str(
    ...
    def _node_payload(self, node: Any) -> Dict[str, Any]:
