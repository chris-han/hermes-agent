...
(function () {
  ...
  const MARKDOWN_ALLOWED_TAGS = new Set([
    "A", "CODE", "EM", "H1", "H2", "H3", "H4", "LI", "P", "PRE", "STRONG", "UL",
  ]);
  ...
  function sanitizeMarkdownHtml(html) {
    ...
    for (const node of nodes) {
      ...
      const allowedAttrs = MARKDOWN_ALLOWED_ATTRS[node.tagName] || new Set();
  ...
  function KanbanPage() {
    ...
    useEffect(function () {
      ...
      async function openWs() {
        ...
        let url;
        try { url = await SDK.buildWsUrl(`${API}/events`, qsParams); } catch (_e) { return; }
        if (wsClosedRef.current) return;
        let ws;
