...
class GatewayStreamConsumer:
    ...
    def __init__(
        ...
        self._fallback_preserve_partial = False
    ...
    async def _edit_message(
        ...
        *,
        ...
        try:
            ...
            if "finalize" in params or accepts_kwargs:
                kwargs["finalize"] = finalize
            if self.metadata:
                if "metadata" in params or accepts_kwargs:
    ...
    def _raw_message_limit(self) -> int:
        """Return the streaming overflow limit for raw response text."""
        raw_limit = getattr(self.adapter, "MAX_MESSAGE_LENGTH", 4096)
        hook = getattr(self.adapter, "streaming_overflow_limit", None)
        if callable(hook):
            try:
                rich_limit = hook()
            except Exception:
                rich_limit = None
            if isinstance(rich_limit, int) and rich_limit > 0:
                return rich_limit
        return raw_limit if isinstance(raw_limit, int) and raw_limit > 0 else 4096

    def _send_metadata(self, *, finalize: bool = False) -> dict:
    ...
    async def _send_new_chunk(self, text: str, reply_to_id: Optional[str]) -> Optional[str]:
        ...
        try:
            ...
            if result.success and result.message_id:
                ...
                self._preview_message_ids.add(str(result.message_id))
    ...
    async def _send_fallback_final(self, text: str) -> None:
        ...
        for chunk in chunks:
            ...
            for attempt in range(2):
                result = await self.adapter.send(
                    chat_id=self.chat_id,
                    content=chunk,
                    metadata=self._send_metadata(finalize=True),
                )
                if result.success:
                    break
        ...
        if (
            ...
            and not self._fallback_preserve_partial
        ):
    ...
    def _should_send_fresh_final(self) -> bool:
        ...
        if threshold <= 0:
            if not isinstance(self.adapter, _BasePlatformAdapter):
                return False
            hook = getattr(self.adapter, "prefers_fresh_final_streaming", None)
            if not callable(hook):
                return False
            try:
                return bool(hook(self._last_sent_text, metadata=self.metadata))
            except TypeError:
                try:
                    return bool(hook(self._last_sent_text))
                except Exception:
                    return False
            except Exception:
                return False
    ...
    async def _try_fresh_final(self, text: str) -> bool:
        ...
        Ported from openclaw/openclaw#72038.
        """
        old_message_ids = set(self._preview_message_ids)
        if self._message_id:
            old_message_ids.add(self._message_id)
        try:
            result = await self.adapter.send(
                chat_id=self.chat_id,
                content=text,
        ...
        delete_fn = getattr(self.adapter, "delete_message", None)
        if delete_fn is not None:
            for old_message_id in sorted(old_message_ids):
        ...
        try:
            if self._message_id is not None:
                if self._edit_supported:
                    ...
                    if result.success:
                        ...
                        if isinstance(raw_response, dict):
                            self._fallback_preserve_partial = bool(
                                raw_response.get("partial_overflow")
                            )
                ...
                result = await self.adapter.send(
                    ...
                    metadata=self._send_metadata(finalize=finalize),
                )
                if result.success:
