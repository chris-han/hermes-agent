        ...
        if target_lower == "origin":
            if origin:
                return cls(
                    ...
                    delivery_adapter_key=origin.delivery_adapter_key,
                    is_origin=True,
                )
            else:
...
class DeliveryRouter:
    ...
    def __init__(
        ...
        adapter_resolver: Optional[Callable[[DeliveryTarget], Any]] = None,
    ):
        """
        ...
        Args:
            ...
            adapter_resolver: Optional callable to resolve account-scoped adapters
        """
        self.config = config
    ...
    def _resolve_adapter(self, target: DeliveryTarget) -> Any:
        """Resolve the adapter for a delivery target."""
        if self.adapter_resolver is not None:
            resolved = self.adapter_resolver(target)
            if resolved is not None:
                return resolved
            if target.delivery_adapter_key:
                raise ValueError(
                    f"No delivery adapter resolved for key {target.delivery_adapter_key}"
                )
