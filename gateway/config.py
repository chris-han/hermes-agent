...
def _rank_weixin_runtime_account(account: dict[str, Any]) -> tuple[int, str, str]:
    """Rank saved Weixin runtime accounts deterministically for startup reuse."""
    state = str(account.get("runtime_session_state") or "").strip().lower()
    active_score = 1 if state in {"active", "connected", "ok"} else 0
    updated_at = str(
        account.get("runtime_session_updated_at")
        or account.get("updated_at")
        or account.get("saved_at")
        or ""
    )
    account_id = str(account.get("account_id") or "").strip()
    return active_score, updated_at, account_id


def _load_semantier_weixin_runtime_account() -> Optional[dict[str, Any]]:
    """Load the preferred Weixin runtime account from Semantier auth storage.

    The canonical Semantier runtime credential cache in `auth.db` is the only
    supported source for Weixin startup credentials.
    """
    try:
    ...
    try:
        ...
        logger.debug("Failed to load Weixin runtime accounts from Semantier auth DB.", exc_info=True)
        return None

    if not candidates:
        return None

    return max(candidates, key=_rank_weixin_runtime_account)
...
class SessionResetPolicy:
    ...
    at_hour: int = 0  # Hour for daily reset (0-23, UTC)
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "SessionResetPolicy":
        ...
        return cls(
            ...
            at_hour=at_hour if at_hour is not None else 0,
...
_PLATFORM_CONNECTED_CHECKERS: dict[Platform, Callable[[PlatformConfig], bool]] = {
    ...
    Platform.WHATSAPP_CLOUD: lambda cfg: True,  # cloud webhook auth is validated by adapter
    Platform.RELAY: lambda cfg: bool(cfg.extra.get("relay_url")),
    ...
    Platform.GOOGLE_CHAT: lambda cfg: bool(cfg.token or cfg.api_key),
}
...
class GatewayConfig:
    ...
    multiplex_profiles: bool = False  # Namespace sessions by active Hermes profile when serving multiple profiles

    # Unauthorized DM policy
    unauthorized_dm_behavior: str = "pair"  # "pair" or "ignore"

    # Active chat session cap. None/0 disables the cap.
    ...
    # days from the in-memory dict. Keeps the store from
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "GatewayConfig":
        ...
        if multiplex_profiles is None and isinstance(data.get("gateway"), dict):
            multiplex_profiles = data["gateway"].get("multiplex_profiles")
        unauthorized_dm_behavior = _normalize_unauthorized_dm_behavior(
            data.get("unauthorized_dm_behavior"),
            "pair",
        )
        max_concurrent_sessions = data.get("max_concurrent_sessions")
        if max_concurrent_sessions is None and isinstance(data.get("gateway"), dict):
            max_concurrent_sessions = data["gateway"].get("max_concurrent_sessions")
...
def _validate_gateway_config(config: "GatewayConfig") -> None:
    ...
    if not (0 <= policy.at_hour <= 23):
        logger.warning(
            "Invalid at_hour=%s (must be 0-23 UTC). Using default 0.", policy.at_hour
        )
        policy.at_hour = 0
...
def _apply_env_overrides(config: GatewayConfig) -> None:
    ...
    saved_weixin_account = _load_semantier_weixin_runtime_account()
    saved_weixin_token = (
        str(saved_weixin_account.get("token") or "").strip()
        if saved_weixin_account
        else ""
    )
    saved_weixin_account_id = (
        str(saved_weixin_account.get("account_id") or "").strip()
        if saved_weixin_account
        else ""
    )
    if saved_weixin_token or saved_weixin_account_id:
        if Platform.WEIXIN not in config.platforms:
            config.platforms[Platform.WEIXIN] = PlatformConfig()
        config.platforms[Platform.WEIXIN].enabled = True
        if saved_weixin_token:
            config.platforms[Platform.WEIXIN].token = saved_weixin_token
        extra = config.platforms[Platform.WEIXIN].extra
        if saved_weixin_account_id:
            extra["account_id"] = saved_weixin_account_id
        weixin_base_url = (
            str((saved_weixin_account or {}).get("base_url") or "").strip()
        )
        if weixin_base_url:
            extra["base_url"] = weixin_base_url.rstrip("/")
        weixin_cdn_base_url = (
            str((saved_weixin_account or {}).get("cdn_base_url") or "").strip()
        )
        if weixin_cdn_base_url:
            extra["cdn_base_url"] = weixin_cdn_base_url.rstrip("/")
        weixin_dm_policy = (
            os.getenv("WEIXIN_DM_POLICY", "").strip().lower()
            or str((saved_weixin_account or {}).get("dm_policy") or "").strip().lower()
        )
        if weixin_dm_policy:
            extra["dm_policy"] = weixin_dm_policy
        weixin_group_policy = (
            os.getenv("WEIXIN_GROUP_POLICY", "").strip().lower()
            or str((saved_weixin_account or {}).get("group_policy") or "").strip().lower()
        )
        if weixin_group_policy:
            extra["group_policy"] = weixin_group_policy
        weixin_allowed_users = (
            os.getenv("WEIXIN_ALLOWED_USERS", "").strip()
            or str((saved_weixin_account or {}).get("allow_from") or "").strip()
        )
        if weixin_allowed_users:
            extra["allow_from"] = weixin_allowed_users
        weixin_group_allowed_users = (
            os.getenv("WEIXIN_GROUP_ALLOWED_USERS", "").strip()
            or str((saved_weixin_account or {}).get("group_allow_from") or "").strip()
        )
        ...
        weixin_home = (
            str((saved_weixin_account or {}).get("home_channel") or "").strip()
        )
        if weixin_home:
    ...
    try:
        ...
        for entry in platform_registry.plugin_entries():
            ...
            existing_config = config.platforms.get(platform, PlatformConfig())
            if (
                not existing_config.enabled
                and existing_config.extra.get("_enabled_explicit")
            ):
                continue
            if not existing_config.enabled and entry.is_connected is not None:
                try:
                    if not entry.is_connected(existing_config):
                        continue
                except Exception as e:
    ...
    for pconfig in config.platforms.values():
        pconfig.extra.pop("_enabled_explicit", None)
