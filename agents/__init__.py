...
def __getattr__(name: str):
    # Delegate attribute resolution to repo modules when available so tests
    # importing symbols from `agents` (e.g., `SemantierAgent`) work even when
    # this shim package is the on-disk package module.
    from importlib import import_module

    if name in {"app", "execute"}:
        module = import_module("agents.gateway")
        return getattr(module, name)
    if name == "SemantierAgent":
        module = import_module("agents.semantier_agent")
        return getattr(module, name)
    if name in {"run_agent_cli", "run_api", "run_gateway_cli", "run_runtime_cli"}:
        module = import_module("agents.launcher")
        return getattr(module, name)
