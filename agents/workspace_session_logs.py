...
def _canonical_module_path() -> Path:
    ...
    candidates = []
    if configured:
        candidates.append(Path(configured).expanduser() / "src/agents/workspace_session_logs.py")
    candidates.extend(parent / "src/agents/workspace_session_logs.py" for parent in current_path.parents)
    for candidate in candidates:
        if candidate.is_file() and candidate.resolve() != current_path:
            return candidate
    raise ModuleNotFoundError(
        "Canonical Semantier workspace_session_logs implementation is unavailable; "
        "set SEMANTIER_RUNTIME_ROOT to the semantier-runtime checkout"
    )
...
def _load_upstream_module() -> ModuleType:
    ...
    if module is not None and Path(module.__file__).resolve() == module_path.resolve():
        return module
...
replace_workspace_session_log_messages = _UPSTREAM.replace_workspace_session_log_messages
...
def _iter_workspace_homes(root: Path) -> Iterable[Path]:
    if not root.exists():
        return []
    return [
        child / ".hermes"
        for child in root.iterdir()
        if child.is_dir() and (child / ".hermes").is_dir()
    ]


def _local_index_matches(
    root: Path,
    *,
    ...
    rows: list[dict[str, Any]] = []
    for workspace_home in _iter_workspace_homes(root):
        try:
        ...
        for session_id, record in sessions.items():
            if not isinstance(record, dict):
                continue
            if canonical_session_id and session_id != canonical_session_id:
                continue
            if platform and record.get("platform") != platform:
                continue
            if chat_id and record.get("chat_id") != chat_id:
                continue
            if thread_id and record.get("thread_id") != thread_id:
                continue
            if origin_user_id and record.get("origin_user_id") != origin_user_id:
                continue
            wanted_key = platform_session_key or alias
            if wanted_key and wanted_key not in {
                record.get("platform_session_key"),
                record.get("alias"),
            }:
                continue
            rows.append(
                {
                    ...
                    "workspace_hermes_home": str(workspace_home),
                    "index_key": session_id,
                    "canonical_session_id": session_id,
                    "session_id": session_id,
                    "session_key": record.get("platform_session_key"),
